...
> usable in the **same** session (from the first prompt). `pom.xml` and the Maven build are
> intentionally untouched. Accept the workspace-trust dialog once so they load.
>
