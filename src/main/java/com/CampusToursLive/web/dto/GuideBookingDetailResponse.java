 ...
 * offering, the participant's user record, and the university — assembled by BookingService.
 *
...
@Schema(
        ...
        description = "A guide-facing booking flattened for the booking inbox.")
public record GuideBookingDetailResponse(
        @Schema(
        ...
        @Schema(
                        description = "Display name of the participant who booked.",
                        example = "Sam Rivera",
                        requiredMode = Schema.RequiredMode.REQUIRED)
                String participantName,
        @Schema(
                        description = "Optional notes the participant left when booking.",
                        example = "First campus visit — please meet at the main gate.",
                        requiredMode = Schema.RequiredMode.NOT_REQUIRED)
                String participantNotes,
        @Schema(
                        description =
                                "ISO-8601 UTC deadline for the guide to respond; null when the"
                                        + " booking has no response deadline.",
