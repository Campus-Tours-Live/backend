...
/**
 * Maps the {@code guide_availability_rules} table (V4__availability_engine_v2.sql) — a guide's
 * recurring weekly availability, encoded as **start + duration** (RFC 5545 / DTSTART+DURATION
 * style): {@code start_local} + {@code window_min}, never {@code start_local} + an end column.
 * There is no 24:00 sentinel and no wraparound — a window that crosses midnight (e.g. start 22:00,
 * window 240 min) is expressed the same way as any other window; only the projection layer (CTL-54
 * Task 2) turns it into a concrete UTC instant range.
 *
